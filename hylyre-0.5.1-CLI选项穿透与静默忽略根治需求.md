...
- 冻结契约：**`contracts_tree_sha256 = cc738c272324022d7ed559340e9c710f9b7f5f94aac62c5dd70042e827a21bae`（本次不得变更）**
...
**位置。** 矩阵测试与 fixture 留在 Hylyre 上游仓库维护运行，不要求随 source 发布件交付；发布件内 `src/hylyre/contracts/` **不得新增或修改任何文件**——该目录受 `contracts_tree_sha256` 保护。
...
- **不改 Step Outcome 协议、trace schema、contracts**。`contracts_tree_sha256` 必须仍为 `cc738c272324022d7ed559340e9c710f9b7f5f94aac62c5dd70042e827a21bae`。P0-1 已据此排除 pre-run reject 形态；P0-2 已据此禁止向 `contracts/` 添加任何文件。
...
- `release.manifest.json` 的 `contracts_tree_sha256` **保持 `cc738c27…` 不变**，并在交付报告中显式声明"契约包本次未变更"；
...
- 发布件内 `src/hylyre/contracts/` 无任何新增或修改文件。

**不得以 0.5.0 同版本替换。** 理由见 §一 P1。

### 7.2 source 发布

发布形态不变：plain-source vendor，无需 wheel；package-data 内协议文档 / Schema / builder 判定表 / golden fixtures 逐字保持。所有权矩阵测试留在上游仓库，不要求随发布件交付。
...
2. **契约逐字节相等**：vendored `src/hylyre/contracts/` 与冻结契约包逐字节比对，重算 `contracts_tree_sha256` 必须仍为 `cc738c27…`。任何差异即退回。
...
5. 版本号、`release.manifest.json` 新 `source.tree_sha256` / file_count / total_bytes、以及 `contracts_tree_sha256` 未变更的显式声明；
