...
> **状态**：**P5 MCP 已交付**；**并行能力**：MITM CA 首版 `hylyre mock push-ca`（`hdc file send`）+ `doctor` 的 `mitmproxy CA (PEM)` 检查；**P2b** 首版 `hylyre bootstrap mock [--install]`。**P4** 已归档；**L5** 已与 §7.5 补严；**`add-cert-bootstrap`** 已归档至 `openspec/changes/archive/2026-05-12-add-cert-bootstrap/`。**下一主目标 P6**（反哺 Skill 6，可选）。
