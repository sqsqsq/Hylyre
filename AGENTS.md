...
1. **若 MCP 可用**：优先用 **`hylyre_run_plan`**、**`hylyre_report_*`**、**`hylyre_doctor`**、**`hylyre_device_list`**、**`hylyre_dump_ui`** / **`hylyre_screenshot`**、**`hylyre_run_*`**（planned JSON：`action` / `tap` / `input` / **`swipe`** / **`scroll`** / **`back`** / **`home`** / **`stop_app`** / **`clear_app`** / **`wait`** / **`wait_for`** / **`wait_gone`** / **`wait_idle`** / **`assert_toast`** / **`start_app`**；若连续多步已定，一次性 **`hylyre_run_steps`**）、**`hylyre_collect_list`**、**`hylyre_find`**、**`hylyre_app_page_*`**（快照 CRUD）、**`hylyre_app_find`**、**`hylyre_app_fingerprint`**，以及按需的 **`hylyre_ai_*`**；路径用**绝对路径**或相对 **仓库根** 的明确路径。
